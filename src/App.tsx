...
import { ToastListener } from './components/ui/useToast';
...
const navItems = [
  ...
  { path: '/admin-dashboard', key: 'admin', icon: '🛡️' },
    ...
    <header className="sticky top-0 z-40 w-full neon-header">
      <div className="mx-auto flex h-14 max-w-7xl items-center px-4">
        ...
        <div className="md:hidden mr-2">
          <DropdownMenu>
            <DropdownMenuTrigger className="inline-flex items-center gap-2 rounded-md bg-white/10 px-3 py-1.5 text-sm text-white hover:bg-white/20">
              ☰ Menu
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              {navItems.map(item => (
                <DropdownMenuItem asChild key={item.path}>
                  <NavLink to={item.path} className="flex items-center gap-2">
                    ...
                    <span className="font-semibold uppercase text-xs">{item.key}</span>
                  </NavLink>
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
        ...
        <nav className="hidden md:flex items-center gap-2 text-sm neon-nav">
          {navItems.map(item => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `px-3 py-2 rounded-md text-white/90 relative transition-all duration-300
                group flex items-center gap-1
                ${isActive ? 'bg-gradient-to-r from-pink-500 to-yellow-400 text-white shadow-lg scale-105 neon-glow' : 'hover:bg-white/10 hover:scale-105'}
                `}
            >
              <span className="mr-1 text-lg transition-transform duration-300 group-hover:scale-125 group-hover:rotate-6">
                {item.icon}
              </span>
              <span className="font-semibold tracking-wide uppercase text-xs">{item.key}</span>
              {/* Animated underline */}
              <span className={`absolute left-2 right-2 -bottom-1 h-0.5 rounded-full transition-all duration-300 ${item.path === window.location.pathname ? 'bg-yellow-400 w-4/5' : 'bg-transparent w-0'}`}></span>
            </NavLink>
          ))}
        </nav>
        <div className="ml-auto flex items-center gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-sm text-white hover:bg-white/20">
              <Avatar userId={user?.uid || 'guest'} size={28} className="shrink-0" />
    ...
    <UserProfileProvider>
      <TooltipProvider delayDuration={300} skipDelayDuration={700}>
        <ToastProvider swipeDirection="right">
          <ToastListener />
          <div className="min-h-screen relative overflow-hidden game-cyberpunk bingo-board-theme">
            ...
            <div className="bingo-balls">
              <div className="bingo-ball ball-b">B</div>
              <div className="bingo-ball ball-i">I</div>
              <div className="bingo-ball ball-n">N</div>
              <div className="bingo-ball ball-g">G</div>
              <div className="bingo-ball ball-o">O</div>
            </div>
            {!isAuthRoute && <Header />}
            <div>
