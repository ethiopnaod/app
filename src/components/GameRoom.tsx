...
const GameRoom: React.FC = () => {
  ...
  // Efficient: Only read active room/user data on login
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setIsLoading(true);
        // Only fetch the active room for this user
        ...
        // Local caching: keep temporary data in memory during gameplay
  ...
  useEffect(() => {
    ...
    return () => {
      // Unsubscribe listeners when users leave the room
  ...
  }, [user, gameId, handleGameEnd, handleGameStart, handleNewNumberCall]);

  // (Removed unused loadGame function)
  ...
  const handleNewNumberCall = useCallback(async (number: number) => {
