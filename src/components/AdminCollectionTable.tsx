...
const AdminCollectionTable: React.FC<AdminCollectionTableProps> = ({ collectionName }) => {
  ...
  if (!docs.length) return <div className="text-yellow-200">No data found in <b>{collectionName}</b>.</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white/10 backdrop-blur-sm rounded-2xl shadow-lg p-6 my-8">
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm text-white">
          <thead>
            <tr>
              {allKeys.map((key: string) => (
                <th key={key} className="px-4 py-2 border-b border-yellow-400/30 text-left font-bold bg-yellow-400/10 text-yellow-400 uppercase tracking-wider">{key}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {docs.map((doc, i) => (
              <tr key={doc.id || i} className="border-b border-yellow-400/10 hover:bg-yellow-400/5 transition-colors">
                {allKeys.map((key: string) => (
      ...
      <div className="flex justify-between items-center mt-4">
        <button
          ...
          disabled={page === 1}
          className="px-4 py-2 bg-gradient-to-r from-yellow-400 to-orange-500 text-white rounded-xl font-bold shadow disabled:opacity-50"
        >
          Previous
        </button>
        <span className="text-yellow-200">Page {page}</span>
        <button
