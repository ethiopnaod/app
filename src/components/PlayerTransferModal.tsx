...
const PlayerTransferModal: React.FC<PlayerTransferModalProps> = ({
  ...
  const handleTransfer = async () => {
    if (!selectedRecipient) {
      const msg = t('pleaseSelectRecipient') || 'Please select a recipient';
      setError(msg);
      toast({ title: t('transferError') || 'Transfer Error', description: msg, status: 'error' });
      return;
    }

    const transferAmount = parseFloat(amount);
    if (isNaN(transferAmount) || transferAmount <= 0) {
      const msg = t('pleaseEnterValidAmount') || 'Please enter a valid amount';
      setError(msg);
      toast({ title: t('transferError') || 'Transfer Error', description: msg, status: 'error' });
      return;
    }
    ...
    if (transferAmount < 1) {
      const msg = t('minimumTransferAmountEtb1') || 'Minimum transfer amount is ETB 1';
      setError(msg);
      toast({ title: t('transferError') || 'Transfer Error', description: msg, status: 'error' });
      return;
    }
  ...
  // Interpolate manually since t() does not support variables directly
  toast({ title: t('transferSuccess') || 'Transfer Success', description: t('transferredEtbTo') ? t('transferredEtbTo').replace('{amount}', transferAmount.toFixed(2)).replace('{name}', selectedRecipient.displayName) : `Transferred ETB ${transferAmount.toFixed(2)} to ${selectedRecipient.displayName}`, status: 'success' });
  onSuccess();
    } catch (error) {
      setError((error as Error).message);
      toast({ title: 'Transfer Error', description: (error as Error).message, status: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
    <DialogTitle>{t('transferFunds') || 'Transfer Funds'}</DialogTitle>
          <DialogClose asChild>
        ...
        <div className="card-cyber p-4 mb-6">
          <div className="text-sm text-slate-400 mb-1">{t('availableBalance') || 'Available Balance'}</div>
                ...
                {searchResults.map((user) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-slate-500">
                      {t('balance') || 'Balance'}: {formatBalance(user.balance)}
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-white mb-2">
            {t('transferAmountEtb') || 'Transfer Amount (ETB)'}
          </label>
          <input
            type="number"
            value={amount}
          ...
          <div className="text-xs text-slate-400 mt-1">
            {t('min') || 'Min'}: {t('etb') || 'ETB'} 1.00 | {t('max') || 'Max'}: {formatBalance(currentBalance)}
          </div>
        </div>
        ...
        {selectedRecipient && amount && parseFloat(amount) > 0 && (
          <div className="bg-slate-800/50 rounded-lg p-4 mb-6">
            <h3 className="text-sm font-medium text-white mb-2">{t('transferSummary') || 'Transfer Summary'}</h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-400">{t('recipient') || 'Recipient'}:</span>
                <span className="text-white">{selectedRecipient.displayName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">{t('amount') || 'Amount'}:</span>
                <span className="text-green-400">ETB {parseFloat(amount).toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">{t('newBalance') || 'New Balance'}:</span>
        ...
        <div className="flex gap-3">
          <DialogClose asChild>
            <button className="flex-1 btn-secondary">{t('cancel') || 'Cancel'}</button>
          </DialogClose>
          <button
          ...
          >
            {isLoading ? t('transferring') || 'Transferring...' : t('transfer') || 'Transfer'}
          </button>
        </div>
      </DialogContent>
    </Dialog>
