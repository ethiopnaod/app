...
interface ChapaPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (data: import('../services/chapaPaymentService').ChapaPaymentResponse['data']) => void;
...
const ChapaPaymentModal: React.FC<ChapaPaymentModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.email) {
      newErrors.email = t('emailIsRequired') || 'Email is required';
    } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
      newErrors.email = t('pleaseEnterValidEmail') || 'Please enter a valid email';
    }

    if (!formData.firstName.trim()) {
      newErrors.firstName = t('firstNameIsRequired') || 'First name is required';
    }

    if (!formData.phone.trim()) {
      newErrors.phone = t('phoneNumberIsRequired') || 'Phone number is required';
    } else if (!/^(\+251|0)?[79]\d{8}$/.test(formData.phone.replace(/\s/g, ''))) {
      newErrors.phone = t('pleaseEnterValidEthiopianPhone') || 'Please enter a valid Ethiopian phone number';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 'success' && response.data?.checkout_url) {
        toast.success(t('paymentInitializedSuccessfully') || 'Payment initialized successfully!');
        if (response.data) {
        ...
        throw new Error(response.message || t('paymentInitializationFailed') || 'Payment initialization failed');
      }
      ...
      <DialogContent className="panel-cyber max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 card-cyber">
          <div className="text-center">
            <p className="text-sm text-slate-400 mb-2">{t('paymentAmount') || 'Payment Amount'}</p>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={t('enterYourLastNameOptional') || 'Enter your last name (optional)'}
                disabled={isLoading}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder={t('examplePhone') || 'e.g., +251912345678 or 0912345678'}
                disabled={isLoading}
              />
              {errors.phone && (
            ...
            <div className="flex items-start gap-3 p-4 card-cyber">
              ...
              <div className="text-sm text-green-800 dark:text-green-200">
                ...
                <p>{t('paymentInfoEncrypted') || "Your payment information is encrypted and secure. We use Chapa's trusted payment gateway."}</p>
              </div>
            </div>
          ...
          <div className="p-6">
            {paymentResponse.status === 'success' ? (
              <div className="flex items-center gap-3 p-4 card-cyber">
                ...
                <div>
                  <p className="font-medium text-green-200">{t('paymentInitialized') || 'Payment Initialized'}</p>
                  <p className="text-sm text-green-300">
                    {t('redirectingToSecurePaymentPage') || 'Redirecting to secure payment page...'}
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 p-4 card-cyber">
                ...
                <div>
                  <p className="font-medium text-red-200">{t('paymentFailed') || 'Payment Failed'}</p>
                  <p className="text-sm text-red-300">
                    {t(paymentResponse.message) || paymentResponse.message}
                  </p>
                </div>
              </div>
            )}
