        ...
        <header className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full flex items-center justify-center shadow-lg overflow-hidden border-4 border-purple-500 animate-spin-slow">
              <Avatar userId={user?.uid || 'guest'} size={64} />
            </div>
            <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {profile && [
              ...
              <div key={mode.title} className="bingo-mode-card rounded-2xl p-6 hover:shadow-[0_0_32px_rgba(0,255,240,0.22),0_0_32px_rgba(255,0,230,0.18)] transform hover:scale-105 transition-all duration-300 cursor-pointer animate-fade-in" style={{ animationDelay: `${idx * 0.15}s` }}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="text-5xl animate-bounce">{mode.icon}</div>
                  <h3 className="text-xl font-extrabold text-white tracking-tight">{mode.title}</h3>
                </div>
                <p className="text-slate-300 mb-5">{mode.desc}</p>
                <div className="flex justify-between items-center">
                  <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 font-semibold border border-emerald-400/30">{mode.price}</span>
                  <span className="text-slate-400 text-sm">2–10 players</span>
                </div>
              </div>
            ))}
          </div>
