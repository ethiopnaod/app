...
const GameLobby: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    return () => console.log('GameLobby unmounted');
  }, []);

  const handleBackToGames = () => {
    console.log('Back to Games button clicked');
    navigate('/game-list');
  };
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-3">
              <Avatar userId={user?.uid || 'guest'} size={32} />
              <span>Player 1 (You)</span>
              <span className="ml-auto text-green-400">Ready</span>
            </div>
            {/* Example other players (static for now) */}
            <div className="flex items-center space-x-3">
              <Avatar userId={'player2'} size={32} />
              <span>Player 2</span>
              <span className="ml-auto text-green-400">Ready</span>
            </div>
            <div className="flex items-center space-x-3">
              <Avatar userId={'player3'} size={32} />
              <span>Player 3</span>
